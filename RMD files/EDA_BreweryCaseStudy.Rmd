...
includePrintIBU
par(mfrow=c(1,2))
head(max_ABV, 5) %>% ggplot(aes(x = state, y = MaxABV, fill=state)) +  geom_histogram(stat = "identity", position = "dodge",  aes(x = state)) +
  labs(title = "Top 5 ABV States", subtitle = "Highest Alcohol by Volume") 

head(max_IBU, 5) %>% ggplot(aes(x = state, y = MaxIBU, fill=state)) +  geom_histogram(stat = "identity", position = "dodge",  aes(x = state)) +
  labs(title = "Top 5 IBU States", subtitle = "Highest International Bitterness Units")
...
beer_by_brew_clean %>% ggplot(aes(x = ABV, fill=State)) +  geom_histogram(stat = "count", position = "dodge",  aes(x = ABV)) +
